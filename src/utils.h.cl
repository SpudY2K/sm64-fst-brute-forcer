...
inline short atan2_lookupG(float z, float x) {
  return (x == 0.0f) ?
    0x0000 :
    short(gArctanTableG[ushort(float(float(z / x) * 1024.0 + 0.5))]);
}
...
inline int find_floor(
  float* position, SurfaceG* private* floor, float& floor_y,
  SurfaceG* floor_set, int n_floor_set) {
  ...
  for (int i = 0; i < n_floor_set; ++i) {
    if (
      x < floor_set[i].min_x || x > floor_set[i].max_x ||
      z < floor_set[i].min_z || z > floor_set[i].max_z) {
      continue;
    }

    if (
      (floor_set[i].vertices[0][2] - z) *
          (floor_set[i].vertices[1][0] - floor_set[i].vertices[0][0]) -
        (floor_set[i].vertices[0][0] - x) *
          (floor_set[i].vertices[1][2] - floor_set[i].vertices[0][2]) <
      0) {
      continue;
    }
    if (
      (floor_set[i].vertices[1][2] - z) *
          (floor_set[i].vertices[2][0] - floor_set[i].vertices[1][0]) -
        (floor_set[i].vertices[1][0] - x) *
          (floor_set[i].vertices[2][2] - floor_set[i].vertices[1][2]) <
      0) {
      continue;
    }
    if (
      (floor_set[i].vertices[2][2] - z) *
          (floor_set[i].vertices[0][0] - floor_set[i].vertices[2][0]) -
        (floor_set[i].vertices[2][0] - x) *
          (floor_set[i].vertices[0][2] - floor_set[i].vertices[2][2]) <
      0) {
      continue;
    }

    float height = -(x * floor_set[i].normal[0] + floor_set[i].normal[2] * z +
...
inline float find_closest_mag(float target) {
  int minIdx = -1;
  int maxIdx = magCount;
...
inline int atan2b(double z, double x) {
