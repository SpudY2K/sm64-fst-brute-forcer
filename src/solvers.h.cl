...
inline bool test_stick_position(
  ...
  float oneUpPlatformZMin, float oneUpPlatformZMax, float oneUpPlatformNormalX,
  float oneUpPlatformNormalY, int f, float* frame1Position,
  float* returnPosition, short (&triangles)[2][3][3], float (&normals)[2][3],
  int d, int q) {
    ...
    if (eqDet > 0) {
      ...
      if (vel1a >= 0) {
        ...
        while (searchLoop) {
          ...
          forward =
            gCosineTableG[intendedDYaw >> 4] * (0.5f + 0.5f * vel1a / 100.0f);
          ...
          if (vel2b > endSpeed) {
            vel1a = fmax(vel1a + 0.25f, nextafter(vel1a, INFINITY));
          }
          else {
            if (vel2b == endSpeed) {
              vel2a      = vel2b;
  ...
  if (
    speedTest && fabs(predictedReturnPosition[0] - returnPosition[0]) < 1000 &&
    ...
    testOneUpPlatformPosition[1] = oneUpPlatformYMin +
      (oneUpPlatformYMax - oneUpPlatformYMin) *
        ((short) (int) testOneUpPlatformPosition[0] - oneUpPlatformXMin) /
        (oneUpPlatformXMax - oneUpPlatformXMin);
    testOneUpPlatformPosition[2] = testOneUpPlatformPosition[2] - zShift;

    if (
      (int) (short) testOneUpPlatformPosition[0] >= oneUpPlatformXMin &&
      ...
      float marioMinY = fmax(-2971.0f, testOneUpPlatformPosition[1] - 78.0f);
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (t >= 0.0 && t <= 1.0) {
          intOnSquishEdge[intersections] = (f == 0 &&
                                            ((i == 0 && squishCeilings[2]) ||
                                             (i == 1 && squishCeilings[0]))) ||
            (f == 1 &&
             ((i == 1 && squishCeilings[1]) || (i == 2 && squishCeilings[3])));
      ...
      double n =
        (gCosineTableG[nextAngle >> 4] + gCosineTableG[angle >> 4]) / 2.0;

      cutPoints[0] =
        ((pz - az) + (n / m) * (ax - px)) / ((bz - az) - (n / m) * (bx - ax));
      ...
      if (cutPoints[0] <= cutPoints[1]) {
        ...
        if (
          fmax(intersectionPoints[0][1], intersectionPoints[1][1]) >
            marioMinY &&
          ...
          if (intersectionPoints[0][1] > testOneUpPlatformPosition[1]) {
            ...
            double ratio =
              (testOneUpPlatformPosition[1] - intersectionPoints[1][1]) /
              (intersectionPoints[0][1] - intersectionPoints[1][1]);
            cutPoints[1] = cutPoints[1] + (cutPoints[0] - cutPoints[1]) * ratio;
            intersectionPoints[1][0] = intersectionPoints[1][0] +
              (intersectionPoints[0][0] - intersectionPoints[1][0]) * ratio;
          ...
          testStartPosition[0] =
            (intersectionPoints[0][0] + intersectionPoints[1][0]) / 2.0f;
          testStartPosition[2] =
            (intersectionPoints[0][2] + intersectionPoints[1][2]) / 2.0f;

          float floorHeight;

          int floorIdx = find_floor(
            testStartPosition, startTriangles, startNormals, &floorHeight);

          if (
            floorIdx == f && floorHeight > marioMinY &&
            floorHeight < testOneUpPlatformPosition[1]) {
            testStartPosition[1]       = floorHeight;
            predictedReturnPosition[0] = testOneUpPlatformPosition[0] +
              (oneUpPlatformNormalY * xVel2a / 4.0f);
            predictedReturnPosition[2] = testOneUpPlatformPosition[2] +
              (oneUpPlatformNormalY * zVel2a / 4.0f);

            if (
              predictedReturnPosition[0] == returnPosition[0] &&
              ...
              printf(
                ...
                testOneUpPlatformPosition[0], testOneUpPlatformPosition[1],
                testOneUpPlatformPosition[2], returnPosition[0],
                returnPosition[1], returnPosition[2], vel1, xVel1, zVel1,
                endSpeed, xVel2a, zVel2a, q, trueX, trueY, cameraYaw,
                startNormals[f][0], startNormals[f][1], startNormals[f][2],
                intersectionPoints[0][0], intersectionPoints[0][1],
                intersectionPoints[0][2], intersectionPoints[1][0],
                intersectionPoints[1][1], intersectionPoints[1][2]);
              printf(
                ...
                platSol.nFrames, platSol.endNormal[0], platSol.endNormal[1],
                platSol.endNormal[2], platSol.endPosition[0],
                platSol.endPosition[1], platSol.endPosition[2], platSol.pux,
                platSol.puz, puSol.angle, puSol.stickMag, puSol.intendedDYaw);
...
inline int calculate_camera_yaw(float* currentPosition, float* lakituPosition) {
  ...
  pan[0] =
    temp[2] * gSineTableG[((65536 + (int) baseCameraFaceAngle) % 65536) >> 4] +
    temp[0] * gCosineTableG[((65536 + (int) baseCameraFaceAngle) % 65536) >> 4];
  pan[2] = temp[2] *
      gCosineTableG[((65536 + (int) baseCameraFaceAngle) % 65536) >> 4] +
    temp[0] * gSineTableG[((65536 + (int) baseCameraFaceAngle) % 65536) >> 4];
    ...
    for (int q = 1; q <= 4; q++) {
      ...
      if (eqDet >= 0) {
        ...
        float frame1Position[3] = {
          oneUpPlatformPosition[0], startPosition[1], oneUpPlatformPosition[2]};
        bool inBoundsTest = true;
        ...
        if (inBoundsTest) {
          ...
          if (angle == angle2) {
            ...
            double stickAngle = fmod(
              65536.0 + fmod(targetDYaw + angle - cameraYaw, 65536.0), 65536.0);
            double stickMagnitude = sqrt(128.0 * targetMag);

            if (stickMagnitude < 70.0) {
              if (stickMagnitude < 64.0) {
                double yS =
                  -stickMagnitude * cos(2.0 * M_PI * (stickAngle / 65536));
                double xS =
                  stickMagnitude * sin(2.0 * M_PI * (stickAngle / 65536));

                for (int x = floor(xS); x <= ceil(xS) && !foundSolution; x++) {
                  if (x != -1 && x != 1) {
                    for (int y = floor(yS); y <= ceil(yS) && !foundSolution;
                         y++) {
                      if (y != -1 && y != 1) {
                        if (test_stick_position(
                              solIdx, x, y, endSpeed, vel1, xVel1, zVel1, angle,
                              cameraYaw, startPosition, oneUpPlatformPosition,
                              oneUpPlatformXMin, oneUpPlatformXMax,
                              oneUpPlatformYMin, oneUpPlatformYMax,
                              oneUpPlatformZMin, oneUpPlatformZMax,
                              oneUpPlatformNormalX, oneUpPlatformNormalY, f,
                              frame1Position, returnPosition, triangles,
                              normals, d, q)) {
                          foundSolution = true;
                        }
                      }
                ...
                if (xS == 0) {
                  if (yS < 0) {
                    for (int y = -64; y <= -128; y--) {
                      if (test_stick_position(
                            solIdx, 0, y, endSpeed, vel1, xVel1, zVel1, angle,
                            cameraYaw, startPosition, oneUpPlatformPosition,
                            oneUpPlatformXMin, oneUpPlatformXMax,
                            oneUpPlatformYMin, oneUpPlatformYMax,
                            oneUpPlatformZMin, oneUpPlatformZMax,
                            oneUpPlatformNormalX, oneUpPlatformNormalY, f,
                            frame1Position, returnPosition, triangles, normals,
                            d, q)) {
                        foundSolution = true;
                        break;
                      }
                  ...
                  if (xS < 0) {
                    for (int x = -64; x <= -128; x--) {
                      if (test_stick_position(
                            solIdx, x, 0, endSpeed, vel1, xVel1, zVel1, angle,
                            cameraYaw, startPosition, oneUpPlatformPosition,
                            oneUpPlatformXMin, oneUpPlatformXMax,
                            oneUpPlatformYMin, oneUpPlatformYMax,
                            oneUpPlatformZMin, oneUpPlatformZMax,
                  ...
                  if (xS < 0) {
                    ...
                    for (int x = floor(xS); x <= maxX; x++) {
                      ...
                      if (fabs(floor(y)) != 1.0) {
                        if (test_stick_position(
                              solIdx, x, floor(y), endSpeed, vel1, xVel1, zVel1,
                              angle, cameraYaw, startPosition,
                              oneUpPlatformPosition, oneUpPlatformXMin,
                              oneUpPlatformXMax, oneUpPlatformYMin,
                              oneUpPlatformYMax, oneUpPlatformZMin,
                              oneUpPlatformZMax, oneUpPlatformNormalX,
                              oneUpPlatformNormalY, f, frame1Position,
                              returnPosition, triangles, normals, d, q)) {
                          foundSolution = true;
                          break;
                        }
                      }
                      ...
                      if (fabs(ceil(y)) != 1.0) {
                        if (test_stick_position(
                              solIdx, x, ceil(y), endSpeed, vel1, xVel1, zVel1,
                              angle, cameraYaw, startPosition,
                              oneUpPlatformPosition, oneUpPlatformXMin,
                              oneUpPlatformXMax, oneUpPlatformYMin,
                              oneUpPlatformYMax, oneUpPlatformZMin,
                  ...
                  if (yS < 0) {
                    ...
                    for (int y = floor(yS); y <= maxY; y++) {
                      ...
                      if (fabs(floor(x)) != 1.0) {
                        if (test_stick_position(
                              solIdx, floor(x), y, endSpeed, vel1, xVel1, zVel1,
                              angle, cameraYaw, startPosition,
                              oneUpPlatformPosition, oneUpPlatformXMin,
                              oneUpPlatformXMax, oneUpPlatformYMin,
                              oneUpPlatformYMax, oneUpPlatformZMin,
                      ...
                      if (fabs(ceil(x)) != 1.0) {
                        if (test_stick_position(
                              solIdx, ceil(x), y, endSpeed, vel1, xVel1, zVel1,
                              angle, cameraYaw, startPosition,
                              oneUpPlatformPosition, oneUpPlatformXMin,
                              oneUpPlatformXMax, oneUpPlatformYMin,
                              oneUpPlatformYMax, oneUpPlatformZMin,
...
inline bool find_10k_route(int solIdx, int f, int d, int h) {
  ...
  for (int i = minXPU; i <= maxXPU; i++) {
    ...
    int minZPU = (int) ceil(
      (fmin(z0, z1) - (oneUpPlatformZMax + oneUpPlatformBuffer)) / 65536.0);
    int maxZPU = (int) floor(
      (fmax(z0, z1) - (oneUpPlatformZMin - oneUpPlatformBuffer)) / 65536.0);
    ...
    for (int j = minZPU; j <= maxZPU; j++) {
      ...
      double x0 = signX *
          sqrt(r * r -
               (j * 65536.0 + (oneUpPlatformZMin - oneUpPlatformBuffer) -
                sol->returnPosition[2]) *
                 (j * 65536.0 + (oneUpPlatformZMin - oneUpPlatformBuffer) -
                  sol->returnPosition[2])) +
        sol->returnPosition[0];
      double x1 = signX *
          sqrt(r * r -
               (j * 65536.0 + (oneUpPlatformZMax + oneUpPlatformBuffer) -
                sol->returnPosition[2]) *
                 (j * 65536.0 + (oneUpPlatformZMax + oneUpPlatformBuffer) -
                  sol->returnPosition[2])) +
        sol->returnPosition[0];
      ...
      double minX = fmax(
        fmin(x0, x1), i * 65536.0 + (oneUpPlatformXMin - oneUpPlatformBuffer));
      double maxX = fmin(
        fmax(x0, x1), i * 65536.0 + (oneUpPlatformXMax + oneUpPlatformBuffer));
      ...
      if (test_one_up_position(
            solIdx, startPosition, oneUpPlatformPosition, sol->returnPosition,
            returnSpeed, oneUpPlatformXMin, oneUpPlatformXMax,
            oneUpPlatformYMin, oneUpPlatformYMax, oneUpPlatformZMin,
            oneUpPlatformZMax, oneUpPlatformNormalX, oneUpPlatformNormalY, f, d,
            sol->endTriangles, sol->endTriangleNormals)) {
        foundSolution = true;
      }
    }
  }
...
inline void find_pu_slide_setup(PlatformSolution* sol, int solIdx) {
  float floorHeight;
  int floorIdx = find_floor(
    sol->endPosition, sol->endTriangles, sol->endTriangleNormals, &floorHeight);
  ...
  if (floorIdx != -1) {
    ...
    for (int i = 0; i < 8192; i++) {
      ...
      double n =
        (-(s * t) - 1.0 + sqrt((s * t - 1.0) * (s * t - 1.0) + 4.0 * s * s)) /
        (2.0 * s);
      ...
      if (fabs(n1) <= 1.01) {
        ...
        for (int j = minAngle; j <= 32768 - minAngle; j += 16) {
          ...
          float positionTest[3] = {
            sol->endPosition[0], sol->endPosition[1], sol->endPosition[2]};

          for (int s = 0; s < 4; s++) {
            positionTest[0] = positionTest[0] +
              sol->endTriangleNormals[floorIdx][1] * (xVel1a / 4.0f);
            positionTest[2] = positionTest[2] +
              sol->endTriangleNormals[floorIdx][1] * (zVel1a / 4.0f);
          }

          int floorIdx1 = find_floor(
            positionTest, sol->endTriangles, sol->endTriangleNormals,
            &floorHeight);
...
inline void platform_logic(
  ...
  float invsqrt = 1.0f /
    sqrt(mat[1][0] * mat[1][0] + mat[1][1] * mat[1][1] + mat[1][2] * mat[1][2]);
  ...
  invsqrt = 1.0f /
    sqrt(mat[0][0] * mat[0][0] + mat[0][1] * mat[0][1] + mat[0][2] * mat[0][2]);
  ...
  invsqrt = 1.0f /
    sqrt(mat[2][0] * mat[2][0] + mat[2][1] * mat[2][1] + mat[2][2] * mat[2][2]);
  ...
  platform_normal[0] = (platform_normal[0] <= dx) ?
    ((dx - platform_normal[0] < 0.01f) ? dx : (platform_normal[0] + 0.01f)) :
    ((dx - platform_normal[0] > -0.01f) ? dx : (platform_normal[0] - 0.01f));
  platform_normal[1] = (platform_normal[1] <= dy) ?
    ((dy - platform_normal[1] < 0.01f) ? dy : (platform_normal[1] + 0.01f)) :
    ((dy - platform_normal[1] > -0.01f) ? dy : (platform_normal[1] - 0.01f));
  platform_normal[2] = (platform_normal[2] <= dz) ?
    ((dz - platform_normal[2] < 0.01f) ? dz : (platform_normal[2] + 0.01f)) :
    ((dz - platform_normal[2] > -0.01f) ? dz : (platform_normal[2] - 0.01f));
...
inline bool try_pu_xz(
  ...
  float mario_pos[3]   = {
    float(x + position[0]), position[1], float(z + position[2])};
    ...
    for (int i = 0; i < n_y_ranges; i++) {
      if (mario_pos[1] >= *(lower_y + i) && mario_pos[1] < *(upper_y + i)) {
        ...
        for (int f = 0; f < n_floor_ranges; f++) {
          if (
            mario_pos[1] >= lower_floor[f] && mario_pos[1] <= upper_floor[f]) {
            good_solution = true;
            break;
          }
        }
        ...
        if (!good_solution) {
          ...
          int closest_pu_dist = fmin(
            fmin(mario_pos[0] + two_p31, two_p31 - 1.0f - mario_pos[0]),
            fmin(mario_pos[2] + two_p31, two_p31 - 1.0f - mario_pos[2]));

          if (closest_pu_dist >= speed / 4.0) {
            int total_falling_frames = (int) floor(
              (two_p32 - closest_pu_dist - 3.0 * speed / 2.0) / speed);
...
inline bool try_pu_x(
  float* normal, float* position, short (&current_triangles)[2][3][3],
  float (&triangle_normals)[2][3], float (&T_start)[4][4],
  float (&T_tilt)[4][4], double x, double x1_min, double x1_max, double x2_min,
  double x2_max, double platform_min_x, double platform_max_x,
  ...
  if (
    pu_platform_max_x > fmin(x1_min, x1_max) &&
  ...
  if (
    pu_platform_max_x > fmin(x2_min, x2_max) &&
  ...
  if (min_needed_speed > max_speed) {
    ...
    for (double z = fmin(fmin(0.0, max_z_pu), -min_pu_oob_z);
      ...
      double bpd_x_mod = static_cast<short>(
        static_cast<int>(x + platform_pos[0] + base_platform_displacement_x));
      double bpd_z_mod = static_cast<short>(
        static_cast<int>(z + platform_pos[2] + base_platform_displacement_z));

      // Check if our likely horizontal platform displacement puts us out
      // of bounds. If so, skip checking this PU.
      if (
        fabs(bpd_x_mod) < 8192 + disp_leeway &&
        fabs(bpd_z_mod) < 8192 + disp_leeway) {
        if (!try_pu_xz(
              normal, position, current_triangles, triangle_normals, x, z, nx,
              ny, nz, tilt_idx, q_steps, max_speed, partialSolution)) {
          break;
        }
      }
    }
...
inline bool try_pu_z(
  float* normal, float* position, short (&current_triangles)[2][3][3],
  float (&triangle_normals)[2][3], float (&T_start)[4][4],
  float (&T_tilt)[4][4], double z, double z1_min, double z1_max, double z2_min,
  double z2_max, double platform_min_x, double platform_max_x,
  double platform_min_z, double platform_max_z, double m, double c_min,
  double c_max, double nx, double ny, double nz, int tilt_idx, int q_steps,
  double max_speed, struct PlatformSolution& partialSolution) {
  ...
  if (
    pu_platform_max_z > fmin(z1_min, z1_max) &&
  ...
  if (
    pu_platform_max_z > fmin(z2_min, z2_max) &&
...
inline void try_normal(
  ...
  double platform_min_x = fmin(
    fmin(
      (double) current_triangles[0][0][0], (double) current_triangles[0][1][0]),
    fmin(
  ...
  double platform_min_z = fmin(
    fmin(
      (double) current_triangles[0][0][2], (double) current_triangles[0][1][2]),
    fmin(
  ...
  double max_y = fmax(
    fmax(
      (double) current_triangles[0][0][1], (double) current_triangles[0][1][1]),
    fmax(
      (double) current_triangles[0][2][1],
      (double) current_triangles[1][2][1]));
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (int j = 0; j < n_y_ranges; j++) {
      ...
      if (nx == 0) {
        if (i % 2 == 0) {
          x1_min = (c_min + tan(a[i]) * platform_pos[0] - platform_pos[2]) /
            (tan(a[i]) - m);
          x1_max = (c_max + tan(a[i]) * platform_pos[0] - platform_pos[2]) /
            (tan(a[i]) - m);
          x2_min = 0;
          x2_max = 0;
          ...
          x1_max = 0;
          x2_min =
            (c_min + tan(a[(i + 1) % 4]) * platform_pos[0] - platform_pos[2]) /
            (tan(a[(i + 1) % 4]) - m);
          x2_max =
            (c_max + tan(a[(i + 1) % 4]) * platform_pos[0] - platform_pos[2]) /
            (tan(a[(i + 1) % 4]) - m);
        ...
        if (fabs(m) < 1) {
          ...
          for (double x = fmin(0.0, last_x_pu);
            ...
            if (!try_pu_x(
                  ...
                  T_start, T_tilt, x, x1_min, x1_max, x2_min, x2_max,
                  platform_min_x, platform_max_x, platform_min_z,
                  platform_max_z, m, c_min, c_max, nx, ny, nz, i, q, max_speed,
                  partialSolution)) {
              break;
            }
          }
          ...
          double z1_min =
            tan(a[i]) * x1_min + platform_pos[2] - tan(a[i]) * platform_pos[0];
          double z1_max =
            tan(a[i]) * x1_max + platform_pos[2] - tan(a[i]) * platform_pos[0];
          double z2_min = tan(a[(i + 1) % 4]) * x2_min + platform_pos[2] -
            tan(a[(i + 1) % 4]) * platform_pos[0];
          double z2_max = tan(a[(i + 1) % 4]) * x2_max + platform_pos[2] -
            tan(a[(i + 1) % 4]) * platform_pos[0];
          ...
          for (double z = fmin(0.0, last_z_pu);
            ...
            if (!try_pu_z(
                  normal, position, current_triangles, triangle_normals,
                  T_start, T_tilt, z, z1_min, z1_max, z2_min, z2_max,
                  platform_min_x, platform_max_x, platform_min_z,
                  platform_max_z, m, c_min, c_max, nx, ny, nz, i, q, max_speed,
                  partialSolution)) {
              break;
...
inline void try_position(float* marioPos, float* normal, int maxFrames) {
  ...
  if (
    floor_idx != -1 && floor_height - 100.0f > -3071.0f &&
    ...
    for (int f = 0; f < maxFrames; f++) {
      ...
      onPlatform = floor_idx != -1 && fabs(marioPos[1] - floor_height) <= 4.0;
      ...
      if (onPlatform && oldOnPlatform) {
        ...
        if (testNormal[0] > testNormal[1] || testNormal[2] > testNormal[1]) {
          ...
          float sqrt3 =
            sqrt(testNormal[1] * testNormal[1] + testNormal[0] * testNormal[0]);
          float sqrt4 = sqrt(
            testNormal[1] * testNormal[1] + testNormal[0] * testNormal[0] +
            testNormal[2] * testNormal[2]);

          float result = sign * d * sqrt1 * sqrt3 *
            (d * sqrt2 * (sqrt1 * testNormal[0] - a * sqrt3) * sqrt4 +
             c *
